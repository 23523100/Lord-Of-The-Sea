Shader "GapperGames/Volumetric_Water"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // === FUNGSI TAMBAHAN: FIX UNTUK ERROR BUILDVARYINGS ===
            Varyings BuildVaryings(Attributes input)
            {
                ...
                // Transform ke World Space dan Clip Space (Wajib untuk URP)
                output.positionWS = TransformObjectToWorld(vertexDescription.Position);
                output.positionCS = TransformWorldToHClip(output.positionWS);
                
                // Normal dan Tangent
                output.normalWS = TransformObjectToWorldNormal(vertexDescription.Normal);
                output.tangentWS = float4(TransformObjectToWorldDir(vertexDescription.Tangent), input.tangentOS.w);
                
                // View Direction
                output.viewDirectionWS = GetWorldSpaceViewDir(output.positionWS);

                // Lighting & Shadow Coordination
                #if defined(LIGHTMAP_ON)
